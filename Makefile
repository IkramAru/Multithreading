...
BPF_CFLAGS := -O2 -g -Wall -target bpf -D__TARGET_ARCH_x86 \
              ...
              -Xclang -fdebug-compilation-dir=/usr/local/lib/btf

USER_CFLAGS := -O2 -g -Wall $(shell pkg-config --cflags libbpf) -fopenmp
USER_LDFLAGS := $(shell pkg-config --libs libbpf) -lelf -lz -latomic -lpthread -fopenmp
...
$(BPF_OBJ): src/bpf/flow_xdp.bpf.c include/flow_common.h vmlinux.h
...
clean:
	rm -f $(BPF_OBJ) $(USER_BIN) $(SKEL_HDR) vmlinux.h
